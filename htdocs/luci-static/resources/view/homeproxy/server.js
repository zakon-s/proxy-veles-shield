...
function renderStatus(isRunning, features) {
	...
	let renderHTML;
	if (isRunning)
		renderHTML = spanTemp.format('green', _('Veles Proxy Server'), coreStr, _('RUNNING'));
	else
		renderHTML = spanTemp.format('red', _('Veles Proxy Server'), coreStr, _('NOT RUNNING'));
...
return view.extend({
	...
	render(data) {
		...
		m = new form.Map('homeproxy', _('Veles Proxy Server'),
