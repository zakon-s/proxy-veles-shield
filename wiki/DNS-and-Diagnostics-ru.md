...
Две связанные темы: как Veles Proxy резолвит имена (и почему это важно для утечек) и как проверить, что всё работает.
...
Страница **Диагностика** (Сервисы → Veles Proxy → Диагностика) запускает живые проверки. У каждой карточки своя кнопка **Проверить**.
