...
Есть четыре способа добавить узлы в Veles Proxy. Все они — на **Настройки узлов → Узлы** (кнопки импорта) и **Настройки узлов → Подписки**.
...
Veles Proxy полностью декодирует share-формат Amnezia `vpn://` (`base64url(qCompress(zlib JSON))`) прямо в браузере — без вспомогательных утилит. Он читает контейнер внутри и автоматически распознаёт **оба** типа конфигов Amnezia:
...
Содержимое подписок и share-ссылок очень часто закодировано в **base64** — это норма, а не редкий случай. Veles Proxy декодирует base64 прозрачно:
...
На вкладке **Подписки** добавьте одну или несколько **ссылок подписок**. Роутер скачивает их и поддерживает список узлов в актуальном состоянии. Veles Proxy автоматически распознаёт несколько форматов ответа:
